#!/usr/bin/env python
"""
Script para crear un webhook de items.update en Loyverse

Este script utiliza la API de Loyverse para crear un webhook que notificará
sobre cambios en los productos (creación, modificación, eliminación).
"""
...
def create_items_webhook(webhook_url=None):
    """
    Crea un webhook para items.update en Loyverse
    """
    # Si no se proporciona URL, usar la URL de la aplicación + /webhook/
    if not webhook_url:
        print("Error: Debe proporcionar una URL para el webhook")
        print("Ejemplo: python create_items_webhook.py --url https://miapp.com/webhook/")
        return False
    
    # En desarrollo local, permitimos URLs de ngrok para pruebas
    if not webhook_url.startswith('https://'):
        print("Advertencia: La URL del webhook debe usar HTTPS en producción.")
        print("Sin embargo, para pruebas locales, continuaremos con la URL proporcionada.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Crear webhook de items.update en Loyverse')
    parser.add_argument('--url', type=str, help='URL del webhook (debe usar HTTPS en producción)')
    
    args = parser.parse_args()
    
    create_items_webhook(args.url) 