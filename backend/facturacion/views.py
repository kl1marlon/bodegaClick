...
class FacturaViewSet(viewsets.ModelViewSet):
    ...
    def procesar_factura(self, request, pk=None):
        ...
        if result_precios['success'] and result_inventario['success']:
            return Response({
                'success': True,
                'message': 'Factura procesada correctamente en Loyverse'
            })
        else:
            return Response({
                'success': False,
                'message': 'Error al procesar factura',
                'detail_precios': result_precios.get('message', ''),
                'detail_inventario': result_inventario.get('message', '')
            }, status=status.HTTP_400_BAD_REQUEST)
    
    @action(detail=False, methods=['get'])
    def descargar_reporte_pdf(self, request):
        """
        ...
        if not queryset.exists():
            ...
            for factura in queryset:
                ...
                elements.append(Paragraph(f'Moneda: {dict(Factura.MONEDA_CHOICES)[factura.moneda]}', normal_style))
